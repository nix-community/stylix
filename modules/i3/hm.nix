...
mkTarget {
  ...
  generalConfig =
    { cfg }:
    {
      lib.stylix.i3.bar = lib.warn "stylix: `config.lib.stylix.i3.bar` has been renamed to `config.stylix.targets.i3.exportedBarConfig` and will be removed after 26.11." cfg.exportedBarConfig;
    };
  ...
  configElements = [
    (
      ...
      in
      {
        xsession.windowManager.i3.config.fonts = { inherit names size; };
        stylix.targets.i3.exportedBarConfig.fonts = { inherit names size; };
      }
    )
    (
      { colors }:
