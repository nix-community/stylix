...
mkTarget {
  ...
  extraOptions = {
    ...
    pluginConfigs = lib.mkOption {
      type = lib.types.attrs;
      default = {
        "base16-nvim".colorschemes.enable = true;
        "mini.base16".plugins.mini.enable = true;
      };
      internal = true;
    };
  };
    ...
    (
      { cfg }:
      {
        stylix.targets.nixvim = {
          ...
          exportedModule.config = cfg.pluginConfigs.${cfg.plugin};
        };
      }
      // (lib.optionalAttrs (options.programs ? nixvim && cfg.exportedModule ? config)
        {
          programs.nixvim = cfg.exportedModule.config;
        }
      )
    )
  ];
